...
class GeminiDeepfakeDetector:
    ...
    def __init__(self, api_key: str):
        try:
            ...
            logger.info("✅ Gemini Pro 2.5 detector initialized successfully")
...
REASONING: [detailed analysis in 2-3 sentences]
            ...
            for i, frame_data in enumerate(frames):
                try:
                    ...
                    logger.warning(f"Gemini frame {i+1} analysis timed out")
                    results.append({'confidence': 50.0, 'prediction': 'Timeout', 'analysis': 'Analysis timed out'})
                except Exception as e:
                    logger.warning(f"Gemini frame {i+1} analysis failed: {e}")
            ...
            return {
                ...
                'model_type': 'Gemini Pro 2.5 with Context Prompting',
                'frame_analyses': results,
                'frames_analyzed': len(frames),
                'ai_analysis': {
                    'technical_reasoning': f'Advanced Gemini Pro 2.5 analysis with specialized deepfake detection prompting on {len(frames)} frames',
            ...
            logger.warning("Gemini analysis timed out completely")
