...
# **FIXED** - MTCNN completely disabled to prevent segmentation faults
MTCNN_AVAILABLE = False
MTCNN_DETECTOR = None
print("⚠️ MTCNN disabled to prevent segmentation faults - using YOLOv8 and OpenCV fallbacks")
        ...
        if self.yolo_model:
            try:
                ...
                try:
                    ...
                    if "INTERNAL ASSERT FAILED" in str(test_error) or "cuda" in str(test_error).lower():
                        logger.info("  YOLOv8: Forcing CPU mode due to CUDA error")
                        self.yolo_model.to("cpu")
                        try:
                            with torch.no_grad():
                                _ = self.yolo_model(test_input, verbose=False)
                            logger.info("  YOLOv8 test: [OK] Working on CPU")
                        except Exception as cpu_error:
                            logger.warning(f"  YOLOv8 test: [ERROR] CPU mode also failed - {cpu_error}")
