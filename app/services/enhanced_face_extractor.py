    ...
    def __init__(self):
        ...
        if self.yolo_available:
            try:
                ...
                logger.info("✅ YOLOv8 Face Detection loaded (99.1% accuracy)")
    ...
    async def extract_faces_optimized(self, video_path: str, **kwargs) -> List[torch.Tensor]:
        ...
        if self.yolo_available and self.yolo_model:
            try:
                ...
                if faces:
                    logger.info(f"✅ YOLOv8 extracted {len(faces)} faces")
                    return faces
                else:
                    logger.info("⚠️ YOLOv8 found no faces, falling back to your existing method")
            except Exception as e:
                logger.warning(f"⚠️ YOLOv8 extraction failed: {e}, using fallback")
        
        # Fallback to your existing system (no changes needed)
        logger.info("🔄 Using your existing face extraction method...")
        faces = fallback_extractor(video_path, **kwargs)
        logger.info(f"✅ Fallback extracted {len(faces)} faces")
        return faces
            ...
            if frame_count % 3 == 0:  # Process every 3rd frame for speed
                ...
                for face_crop in face_data.get('face_crops', [])[:3]:  # Max 3 faces per frame
                    try:
                        ...
                        from backend.app.services.video_processor import preprocess_face_crop
