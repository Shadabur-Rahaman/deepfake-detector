...
class ModernAIContentDetector:
    ...
    def _ensemble_modern_ai_decision(self, pixel_artifacts, temporal_artifacts, 
        ...
        final_score = (
            pixel_artifacts * 0.35 +
            temporal_artifacts * 0.25 +
            frequency_artifacts * 0.20 +
            np.mean(list(tool_scores.values())) * 0.20
        )
        ...
        if final_score >= 0.6:
            ...
            confidence = min(final_score * 100, 95.0)
        else:
            prediction = "Authentic Content"
            confidence = min((1.0 - final_score) * 100, 95.0)
        
        return {
            'prediction': prediction,
            'confidence': confidence,
            'likely_tool': likely_tool if final_score >= 0.6 else 'none',
