...
class RealTimeDeepfakeDetector:
    ...
    def __init__(self):
        ...
        print(f"🔧 RealTimeDetector initialized on device: {self.device}")
    ...
    async def load_model(self):
        ...
        try:
            if self.model is None:
                ...
                print("✅ Real-time detector model loaded successfully")
    ...
    async def real_time_analyze(self, frame: np.ndarray) -> Dict:
        ...
        try:
            ...
            try:
                ...
                print(f"❌ Face preprocessing error: {prep_error}")
                return {
    ...
    def _detect_faces_enhanced(self, frame: np.ndarray) -> list:
        ...
        try:
            ...
            try:
                ...
                print(f"⚠️ Frontal face detection failed: {e}")
            ...
            if not faces and self.profile_cascade:
                try:
                    ...
                    print(f"⚠️ Profile face detection failed: {e}")
            ...
            if not faces:
                try:
                    ...
                    print(f"⚠️ Sensitive face detection failed: {e}")

            # **DEBUG**: Log detection details
            if faces:
                print(f"✅ Face detection successful: {len(faces)} faces found")
                for i, face in enumerate(faces):
                    print(f"  Face {i+1}: {face['type']} at ({face['x']},{face['y']}) size {face['w']}x{face['h']} confidence {face['confidence']}")
            else:
                print(f"❌ No faces detected with any method")
