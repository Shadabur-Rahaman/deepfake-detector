...
#         try:
#             if not os.path.exists(MODEL_PATH):
#                 logger.warning("[WARNING] Model file not found. Creating demo model.")
...
#             print(f"[TEST] MODEL OUTPUT: {avg_prob:.4f}")
...
#             print(f"[DATA] MODEL ANALYSIS for {video_filename}:")
...
#                 print(f"   ⭐ Current avg: {avg_prob:.4f} - {'[OK] CORRECT' if avg_prob < 0.5 else '[ERROR] WRONG'}")
#             else:
...
#         logger.info(f"[TEST] MODEL OUTPUT: {avg_prob:.4f}")
#         logger.info(f"[OK] DETECTION RESULT: {result} (Confidence: {confidence:.3f})")
...
# app/services/deepfake_detector.py - FINAL STABLE VERSION
import numpy as np
import os
import torch
import torch.nn as nn
...
class DeepfakeDetector:
    def __init__(self, model_path: str, device: str = 'auto'):
        ...
        try:
            ...
            logger.error(f"[ERROR] Failed to initialize model from {model_path}: {e}", exc_info=True)
    ...
    def _load_model(self, model_path: str):
        logger.info(f"Loading EfficientNet-B0 model from: {model_path}")
    ...
    def predict(self, faces: List[torch.Tensor]) -> Dict:
        if not self.is_loaded or self.model is None:
            return {"prediction": "Analysis Failed", "confidence": 0.0, "error": "Model not loaded."}
        if not faces:
        ...
        try:
            for face_tensor in faces:
                ...
                # FINAL GRAYSCALE FIX: Ensure tensor has 3 channels for the model
                processed_tensor = face_tensor
                if processed_tensor.shape[0] == 1:
                    processed_tensor = processed_tensor.repeat(3, 1, 1)

                with torch.no_grad():
                    face_batch = processed_tensor.unsqueeze(0).to(self.device)
                    logit = self.model(face_batch)
                    probability = torch.sigmoid(logit).cpu().item()
                    all_probabilities.append(probability)

            if not all_probabilities:
                return {"prediction": "Analysis Failed", "confidence": 0.0, "error": "No valid faces were processed."}

            avg_prob = float(np.mean(all_probabilities))
            is_real = avg_prob >= 0.5
            result_label = "Real Video" if is_real else "Deepfake Detected"
            confidence = avg_prob if is_real else 1.0 - avg_prob

            return {
                "prediction": result_label,
                "confidence": float(confidence) * 100,
                "average_real_probability": avg_prob,
                "faces_analyzed": len(all_probabilities)
            }
        except Exception as e:
            logger.error(f"[ERROR] Prediction failed during analysis: {e}", exc_info=True)
...
class AdvancedDetector:
    ...
    def enhanced_analyze_faces(self, faces: List[torch.Tensor]) -> Dict:
        if not faces:
            return {"prediction": "No Faces Detected", "confidence": 0.0}
        return self.detector.predict(faces)
def load_deepfake_model():
    """Load and return the deepfake detection model"""
    return deepfake_detector_instance.model

deepfake_detector_instance = DeepfakeDetector(model_path=MODEL_PATH)
enhanced_detector = AdvancedDetector(detector=deepfake_detector_instance)

import logging
logger = logging.getLogger(__name__)
logger.info(f"[FIX] deepfake_detector_instance initialized: {deepfake_detector_instance is not None}")
...
#     print("[WARNING] ResNet not available")
...
# print(f"[START] Deepfake Detector initialized - Device: {device}")
...
#         try:
#             logger.info("[LOADING] Loading Advanced Models...")
...
#                             print(f"[OK] YOLO ({variant}) loaded successfully")
#                             break
#                         except Exception:
#                             continue
#                 except Exception as e:
...
#                 except Exception as e:
#                     print(f"[WARNING] ResNet-50 loading failed: {e}")
#                     self.resnet50_model = None
...
#                 self.efficientnet_model = load_deepfake_model()
#                 self.models_loaded = True
#                 print("[LOADING] Fallback to EfficientNet-only mode")
...
#         start_time = time.time()
#         print(f"[START] Starting Advanced Analysis on {len(faces)} faces...")
        
#         try:
#             # Stage 1: Primary EfficientNet Analysis
#             print("[DATA] Stage 1: EfficientNet Analysis...")
...
#             print("[DATA] Stage 2: Face Quality Analysis...")
...
#             print(f"[TIME] Advanced analysis completed in {processing_time:.2f}s")
...
#         except Exception as e:
#             logger.error(f"[ERROR] Advanced detection failed: {e}")
...
#             print(f"   [OK] EfficientNet: {result} (confidence: {confidence:.3f})")
#             return result, confidence
#         except Exception as e:
#             print(f"   [ERROR] EfficientNet analysis failed: {e}")
...
#         except Exception as e:
#             print(f"   [ERROR] Face quality analysis failed: {e}")
#             return 0.4
...
#         except Exception as e:
#             print(f"   [ERROR] Spatial analysis failed: {e}")
#             return 0.4
...
#         logger.info("[LOADING] Using simple detection mode")
#         start_time = time.time()
...
#     print("[START] Advanced Deepfake Detector Module Loaded")
