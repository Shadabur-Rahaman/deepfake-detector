    ...
    def _load_available_models(self):
        ...
        if MODEL_AVAILABILITY.get('mesonet'):
            try:
                ...
                logger.info("[OK] MesoNet loaded (deepfake specialist)")
                ENHANCED_DETECTION_AVAILABLE = True
            except Exception as e:
                logger.warning(f"[WARNING] MesoNet loading failed: {e}")
        logger.info(f"[START] Enhanced engine loaded with {len(self.advanced_models)} advanced models.")
    ...
    async def enhanced_detect(self, faces: List[torch.Tensor]) -> Dict:
        ...
        if 'mesonet' in self.advanced_models:
            try:
                ...
                logger.warning(f"[WARNING] MesoNet prediction failed during runtime: {e}")
