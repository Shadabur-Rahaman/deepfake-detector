...
class ToolSpecificDetectorSuite:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"Veo3 detector initialization failed: {e}")
            self.veo3_detector = None
            
        try:
            self.sora_detector = SoraSpecificDetector()
        except Exception as e:
            logger.warning(f"Sora detector initialization failed: {e}")
            self.sora_detector = None
            
        try:
            self.midjourney_detector = MidjourneySpecificDetector()
        except Exception as e:
            logger.warning(f"Midjourney detector initialization failed: {e}")
            self.midjourney_detector = None
            
        try:
            self.runway_detector = RunwayMLDetector()
        except Exception as e:
            logger.warning(f"Runway detector initialization failed: {e}")
            self.runway_detector = None
            
        try:
            self.stable_diffusion_detector = StableDiffusionDetector()
        except Exception as e:
            logger.warning(f"Stable Diffusion detector initialization failed: {e}")
            self.stable_diffusion_detector = None
    ...
    async def analyze_with_tool_detection(self, faces: List[torch.Tensor],
        ...
        if video_path and self.veo3_detector:
            try:
                ...
                logger.warning(f"Veo3 detection failed: {e}")

        # Sora detection (video-focused)
        if video_path and self.sora_detector:
            try:
                ...
                logger.warning(f"Sora detection failed: {e}")

        # Midjourney detection (image-focused)
        if self.midjourney_detector:
            try:
                ...
                logger.warning(f"Midjourney detection failed: {e}")

        # RunwayML detection
        if self.runway_detector:
            try:
                ...
                logger.warning(f"Runway detection failed: {e}")

        # Stable Diffusion detection
        if self.stable_diffusion_detector:
            try:
                ...
                logger.warning(f"Stable Diffusion detection failed: {e}")
